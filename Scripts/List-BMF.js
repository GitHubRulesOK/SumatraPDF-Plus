...
for (var i = 0; i < scriptArgs.length; i++) {
    ...
    if (part.charAt(0) === "-") {
        ...
        if (key === "o") { outfile = val; continue; } ; print("Unknown switch: " + part); quit();
    }
...
var out = " Mode=MuPDF " + " FILENAME: " + infile; print(out);
...
if (!outline || outline.length === 0) { out = "\n No outline in document: " + infile; print(out); } // no dump() because nothing to dump
  ...
  print ("Lv Page Adobe(MuPDF) file://.pdf#WebUrlFragment  Title")
  print ("== ==== ======================================== = ...")
...
function dump(items, level) {
    for (var i = 0; i < items.length; i++) {
        ...
        var out = pad(level + "", 3) + pad(pageOut + "",5) + pad(it.uri,40) + ' ' + title ;
